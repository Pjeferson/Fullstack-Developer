...
An admin SHALL be able to change any User's role between `default` and
`admin`, and role SHALL NOT be changeable through any other action.

#### Scenario: Changing a role
- **WHEN** an admin sets a User's role to `admin` or `default`
- **THEN** the User's role is updated and the admin is returned to the
  User list
...
- **THEN** it has no effect — the invite and edit actions do not accept a
  `role` parameter at all, so a User's role can only change through the
  dedicated role action

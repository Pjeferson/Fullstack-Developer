...
## Purpose
Establishes the shared visual language (design tokens, reusable UI components) and the
responsive, role-aware navigation shell used across every authenticated page in the app — the
admin area and the self-service profile page alike — so pages present one consistent look
instead of each hand-rolling its own styling.

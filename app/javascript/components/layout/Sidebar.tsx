...
//
// Shared by every authenticated page (admin and self-service alike), not just /admin/* - the
// Users/Imports links only make sense for an admin, so they're shown conditionally. A non-admin
// sees just My profile/Sign out. The brand link's target is role-aware for the same reason: an
// admin's "home" is the User dashboard, a regular User's is their own profile.
export default function Sidebar({ onNavigate }: Props) {
  const { url, props } = usePage<SharedProps>()
  const isAdmin = props.current_user?.role === 'admin'
  const homeHref = isAdmin ? '/admin/users' : '/profile'
  ...
  return (
    <div className="flex h-full flex-col bg-slate-900 text-white">
      <Link href={homeHref} onClick={onNavigate} className="flex items-center gap-2 px-4 py-5">
      ...
      <div className="mt-auto space-y-1 border-t border-slate-800 px-2 py-4">
        <Link
          href="/profile"
          onClick={onNavigate}
